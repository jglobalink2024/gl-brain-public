...
Session: [GL | BRAIN | brain-committer Path Fix · gl-brain Hardening | 260504]

### Changes
- Fixed stale hardcoded repo path in `~/.claude/agents/brain-committer/SKILL.md`: replaced ALL 5 occurrences of `globalink-brain` with `gl-brain`
  - description frontmatter trigger paths
  - Input Contract table ("Relative to gl-brain root")
  - Output section git `cd` block
  - Example Invocation file reference (`gl-brain/command/research.md`)
  - Example Invocation git `cd` block
- Mirrored fix to `globalink-claude-config/agents/brain-committer/SKILL.md` and committed to main (commit 5660fd6)
- Confirmed PR #504 (msitarzewski/agency-agents) already open and up to date on fork branch `jglobalink2024:add/brain-committer`
- Root cause: path discrepancy detected during Hardening #3 (260503) brain writes; gl-brain is the ACTIVE brain, globalink-brain is the older/different structure

### Impact
- brain-committer will now `cd` to the correct `gl-brain` repo on all future invocations
- No functional regressions — fix is path-reference only, no logic changed
...
## 260503 — Trio #1 + RESTORE.md Verification CLOSED [via: CC]
...
- Test B (drift — DRIFT_MARKER_TEST appended to state.md): HARD BANNER fires naming command/state.md with hash prefix diff (expected 9e8c46c0â€¦ â‰  got ed86766aâ€¦) ✅…
...
4. **Test evidence committed** to `command/symphony/hardening-3/test-evidence.md` with full JSON outputs:
   ...
   - Test 2 (drift after appending one space to state.md): HARD BANNER fires citing hash mismatch (expected 9e8c46c0â€¦, got b1028584â€¦) ✅…
...
- **Detection arm (Chat)**: ðŸŸ¡ soft-signal only — date proxy demoted in patterns.md canon due to web_fetch cache. Operators using Chat for brain reads accept residual ~30 min staleness window. CC is the enforcement boundary.
...
- Live Chat smoke test (chat 8494bc07): "Step 1 ✅… POINTER parity (local v3.1 = remote v3.1)" â†’ "L1 Freshness Gate: CLEAR"
...
### ✅… F8a structural fix (catchup â‰  rebless) — IMPLEMENTED + COMMITTED
...
### ðŸ”´ NEW FINDING — Chat-side web_fetch cache gap (Hardening #3 candidate)
...
- Resolve gl-brain â†’ gl-brain-public sync mechanism (PENDING_ACTIONS line about GL_BRAIN_TOKEN — sync didn't propagate corruption commit until manual closeout-style push)
...
Brevo event ID: `<202605040255.72901337518@smtp-relay.mailin.fr>` (subject: "ðŸš¨ CLOSEOUT FAILED — 260503-2254", delivered 22:55:56 EDT)
...
- **Bug A (cosmetic):** `fire_brevo_alert` prints `â†’` (U+2192); Windows charmap fails AFTER HTTP POST succeeds — stderr shows "ALERT failed" as false negative. Fix: `PYTHONUTF8=1` or replace `â†’` with `->`.
...
**Bug A (charmap):** `fire_brevo_alert` Python print line changed `â†’` (U+2192) to `->` (ASCII). Verification output: `ALERT fired -> jason@globalinkservices.io (HTTP 201)` — UnicodeEncodeError eliminated, false-negative "ALERT failed" gone.
...
- F4 — POINTER drift between brain repo + claude.ai project knowledge â†’ CLOSED via Step 1 version parity check
- F8a — Self-sealing freshness signal â†’ CLOSED via integrity.md trust anchor + catchup non-update rule
...
- No nâ‰¥5 threshold — 1-task workspaces claimed adaptive-quality savings with no basis
...
- n â‰¥ 5: per-agent-type adaptive — executions grouped by agent_type, savings computed per bucket, then aggregated. Prevents fast CRM lookups from diluting research task savings.
...
- BYOA api_key save (row 20) — CONFIRMED: UI shows "â— Connected | Last verified: just now | 1 agent polling live"; DB has_key=TRUE for agent-anthropic-1776139354187-0
- Slot 1 Canvas Retest (row 22) — CONFIRMED: execute-step returned success:true with real Claude output; FOUND+FIXED canvasExecution.ts `updated_at` bug (5 occurrences, column doesn't exist â†’ status writes silently failing; fixed to started_at/completed_at)
- Symphony v12 SHIP WITH FIXES verdict (rows 56â€“64) — ALL CLOSED: F1 (BILL-03), F2 (J2 spec), F3 (skeletons), F4 (threshold 70), F5 (J2 spec rewrite); Jason confirmed SHIP WITH FIXES
...
- `ROIPanel.tsx` — replaced "Calculatingâ€¦" text with 4-block animated shimmer matching hero + 2Ã—2 grid
...
- ORACLE-SKILLS-REFERENCE-v2.1 â†’ diff'd against v2.0 spec; v2.1 declared **canonical**
  ...
  Missing line restored: "Interaction: Command bar + FAB + Cmd+K · Toast â†’ Drawer â†’ Intel Log"
...
- NOT in boutique segment (1â€“15 person shops) — COMMAND's beachhead is safe
...
**Brain commits:** `a3ff24c` — command/research.md + command/state.md â†’ pushed gl-brain main ✅“
...
- ORACLE-KB-SPEC-v1.2.md â†’ 12/12 projects ✅“
- session-closeout-SKILL.md â†’ 12/12 (completed prior sessions) ✅“
- PROJECT-IDENTITY.md â†’ 12/12 ✅“ (8 prior sessions + 4 this session: RECON, SHOTO BATCH, SHOTO TRAIN, PERS)
...
**Task 2 — ðŸ¥‡ Canvas Pooled-Key Fallback: SHIPPED (commit f2010ee)**
...
Step 3 RESOLVED: Interaction line was orphaned outside Design System code block in v2.1 initial draft. Surgically appended inside block — exact wording: `Interaction: Command bar + FAB + Cmd+K · Toast â†’ Drawer â†’ Intel Log`. Confirmed clean superset of v2.0. v2.1 now canonical. Changelog note added to file.

Doctrine refresh COMPLETE: ORACLE-SKILLS-REFERENCE-v2.1.md deployed to all 15 current Claude.ai projects (15/15 Ã— HTTP 201). Note: project roster has grown since bootstrap run — 3 new CMD variants (GL-CMD-AOC, GL-CMD-GlobaLink), new PL-AZIMUTH, new GL-TRV, new BRAZ. All 15 received the v2.1 skills reference. Bootstrap kit (ORACLE-KB-SPEC + session-closeout + PROJECT-IDENTITY) coverage on the 5+ new projects (IDs 019dâ€¦) not verified — those projects postdate the bootstrap run and may need kit install.
...
- Network: `POST /api/canvas/execute-step` â†’ HTTP 200 ✅“ — wire is live
...
2. **Wrong UI surface**: "Auto-select agent" routes through `routeAndExecute` (client-side `executeTask`) which does NOT call `triggerAutoHandoff` — Agent B never ran. v12.2 uses "â–¶ Send Task" â†’ TaskBriefCard â†’ "â–¶ Run with {agent}" â†’ `POST /api/tasks/execute` (HTTP endpoint that calls `triggerAutoHandoff` server-side).
...
- `lib/credits.ts` — switched from `increment_starter_credits_used` â†’ `try_deduct_credits` (atomic check+deduct, eliminates TOCTOU)
...
## 260424 — Golden Path Smoke 3Ã— GREEN — Gate Opens

Session: [GL/COMMAND | BUILD | Golden Path Smoke · 3Ã— Green | 260424]
...
- `lib/pipeline/executeTask.ts` — `??` â†’ `||` on `agent.api_key` so empty-string DB default falls through to pooled key; required comment added
- `app/api/dev/smoke/route.ts` — step 8 status check `"completed"` â†’ `"complete"` to match enum; debug logs removed
...
**Blockers fixed**
- Sec-H1 — nuclear reset not atomic â†’ new RPC `dev_nuclear_reset(text)` + `dev_clear_tasks(text)`, SECURITY DEFINER, REVOKE PUBLIC / GRANT service_role. Route calls via `adminClient.rpc()`.
- Sec-H2 — destructive dev actions left no durable trail (dev_logs auto-purges 48h) â†’ `writeDevActionAudit()` writes `event_type='dev_action'` to `audit_ledger` BEFORE each destructive op. clearAudit24h + nuclear RPC exclude dev_action rows so they survive forever.
- Sec-H3 — smoke test had no rate-limit â†’ module-level `activeSmoke` Map (120s TTL) + DB check for existing DEV_SMOKE_TEST tasks in queued/active/executing/in_progress/running; returns 429.
- Code-B1 — smoke route missing `.eq("workspace_id", wid)` on tasks.result + agents reads â†’ added.
- Code-M1 — clear-tasks cascade order had task_chains before tasks â†’ fixed inside RPC: task_executions â†’ agent_handoffs â†’ context_checkpoints â†’ tasks â†’ task_chains.
- DB-H1 — logs route `.order('id', â€¦)` didn't use `(workspace_id, created_at DESC)` index â†’ changed to `.order('created_at', â€¦)`.
- FE-1 — SmokeTest SSE reader leaked on unmount â†’ AbortController + mountedRef + runningRef; reader.cancel on unmount/retrigger; AbortError swallowed silently.
- FE-2 — nuclear `n`-key fired 3Ã— in <200ms â†’ 500ms cooldown via `lastAdvanceAtRef`; strict-mode dedupe via `lastSeenTriggerRef`.
- FE-3 — CONFIRM buttons + nuclear stage held indefinitely â†’ 5s idle auto-reset via dedicated effects with clearTimeout.
...
- `lib/dev-gate.ts` — `requireDevAccess()` returns `{ok, userId, email, workspaceId, workspaceName}` or `{ok:false, status, reason}`. Enforces email âˆˆ `DEV_EMAILS` + workspace ownership via adminClient.
...
- Root cause: trigger fires only when `entry_seq IS NULL` — code was passing `entry_seq: sequence` (non-null) so trigger never ran â†’ race still occurred.
...
**FIX 3 — Anthropic 30sâ†’90s timeout + retry-once + verbose logging**
...
**FIX 1 — lib/ledger.ts: upsert â†’ insert (resolves Postgres 0A000)**
...
- `lib/skills/enforcer.ts` — deleted `nextAuditSeq` helper + converted all 4 sites (`skill_advisory_check`, `skill_violation_blocked` Ã— 3)
...
- **I-3 Sidebar has no Realtime subscription on agents table** — only `audit_ledger` is subscribed (AgentCard.tsx:173-215). Agent status propagates via 5â€“30s polling, so chain auto-fired tasks leave the sidebar stuck until next poll.
- **I-6 audit_ledger mid-refactor** — 19293c6 â†’ `.upsert`, 6845bf5 â†’ reverted 7 sites to `.insert`. Inconsistent across writers.

### Recommended fix order (from doc Â§9)
Batch A (collapse 3 autoHandoff paths â†’ 1) is highest ROI; unblocks cleanup of I-3 and I-5. Then B (server-side agent status + Realtime), C (persist execution_mode + preconfigured_handoff_agent_id migration), D (credit lifecycle rewrite with idempotency key), E (one ledger writer), F (router semantics — semantic stub).
...
**Root CLAUDE.md** — 13 stale references `globalink-brain` â†’ `gl-brain`
...
## 260422 — Model Name Fix: claude-3-5-sonnet-20241022 â†’ claude-sonnet-4-5 (1f7200a)
...
- `lib/store.ts` — toast differentiated: auto â†’ "Executing automatically", manual â†’ "paste the Task Brief to begin"
...
- Timer display now reads: `4.2s · est. 15â€“30s`
...
## Brain Sync Workflow FIXED — allowlist â†’ blocklist (260420)
...
- **Activation tested** in fresh CC session — 3/3 PASS:
  ...
  - **Test 2** (shallow C3 request): Clean refusal — "REFUSED — C3 admits no shallow pass." Cited patterns.md + v12â†’v13 doctrine. Offered three legitimate paths (full C3, C7 relabel, scoped C3 subset).
  ...
  - **Bonus signal**: All three openings included `CALIBER: 15/18 â†’ Opus` — pre-flight rubric internalized, not just cited.
...
- **Brain updates committed**: agent_activity_log.md (zeroth entry) + candidate_agent_specs.md (AGENT 1 â†’ Status: BUILT 260420) — commit `42a2109`
...
- **Doctrine shipped to brain** (d7b5da6): v12.1 patterns delta — C3 categorical, shallow â‰  C3 PASS, deep probe required for P2/P3; public mirror verified
...
Session: [GL | QA | Symphony v12 Execution · 4P Ã— 5J Matrix | 260420]

- **Matrix**: 19 PASS / 1 PARTIAL / 0 FAIL across 20 cells
- **C3 handoff (crown jewel)**: PARTIAL — router stage verified, full Agent A â†’ Agent B chain not walked by harness (F2, harness gap not app regression)
...
- **Auth architecture fixed** — real UI sign-in â†’ storageState (captures sb-<ref>-auth-token cookie); replaces broken localStorage injection that blocked 260419 preflight
...
- **v12 persona-journey matrix design complete**: 4P Ã— 5J Ã— 7C — four behavioral
...
PENDING: Fix BILL-02 (P0 priority — blocks FM pilotâ†’paid path).
...
## v10 Criticals — ALL CLOSED (260414 â†’ 260415)
...
- Fix: continuous 3x multiplier from real duration_ms — manualMinutesPerTask = max(3, actual Ã— 3)
...
- **Brain updates**: agent_activity_log.md (new build entry) + candidate_agent_specs.md (AGENT 2 â†’ Status: BUILT 260420) — commits 834b116, 3212e1b
...
Unmitigated failure modes — pre-SÃ£o Paulo priority:
